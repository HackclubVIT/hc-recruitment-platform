...
function SingleInterviewPageContent() {
  ...
  const fetchInterview = async () => {
    try {
      const res = await fetchApi(`/api/interviews/${id}`)
...
export default function SingleInterviewPage() {
  return (
    <Suspense fallback={<div className="p-8 text-[#bfa8a2] font-mono">LOADING INTERVIEW...</div>}>
      <SingleInterviewPageContent />
    </Suspense>
  );
}