                          ...
                          <button 
                            onClick={async () => {
                              if(confirm("Cancel this interview?")) {
                                await fetch(`${process.env.NEXT_PUBLIC_API_URL || ""}/api/interviews/${interview.id}`, { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ status: "CANCELLED" }) })
