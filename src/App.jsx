...
function App() {
    ...
    if (token) {
      api.getMe()
        .then((data) => {
          ...
          setSessionType((data.user?.role || '').toLowerCase())
  ...
  const handleLogin = (mode) => {
    setAuthenticated(true)
    setSessionType((mode || '').toLowerCase())
    setShowSplash(true)
    setShowLanding(false)
    fetchGlobalData()
  }
  ...
  const isRecruiterPreview =
    ...
    (window.location.pathname === '/recruiter-preview' ||
     window.location.hash === '#recruiter-preview' ||
     window.location.search.includes('preview=recruiter'))

  if (isRecruiterPreview) {
    return <RecruiterDashboard onLogout={() => { window.location.href = '/' }} isPreview={true} />
  }
  ...
  const isRecruiter =
    ...
    globalProfile?.role?.toLowerCase() === 'recruiter';
  const isAdmin =
    sessionType === 'admin' ||
    globalProfile?.role?.toLowerCase() === 'admin';
