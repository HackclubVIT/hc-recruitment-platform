...
export const POST = async (req: Request, res: Response) => {
  try {
    ...
    const lastAnyInterview = await prisma.recruitmentInterview.findFirst({
      where: { application_id: appIdBigInt },
      orderBy: { round: 'desc' }
    });
    let nextRound = 1;
    if (appStatus === "FURTHER_ROUND" && lastAnyInterview) {
      nextRound = lastAnyInterview.round + 1;
    } else {
      const lastActive = await prisma.recruitmentInterview.findFirst({
        where: { application_id: appIdBigInt, status: { not: "CANCELLED" } },
        orderBy: { round: 'desc' }
      });
      nextRound = lastActive ? lastActive.round + 1 : 1;
    }
    ...
    const interview = await prisma.$transaction(async (tx: Prisma.TransactionClient) => {
      ...
      let recruiterIdBigInt: bigint | null = null;
      try {
        recruiterIdBigInt = BigInt(session.id);
      } catch {
        recruiterIdBigInt = null;
      }
      ...
      const newInterview = await tx.recruitmentInterview.create({
        data: {
          application_id: appIdBigInt,
          panel_id,
          recruiter_id: recruiterIdBigInt,
          round: nextRound,
          date: new Date(date),
          start_time: startObj,
          end_time: endObj,
