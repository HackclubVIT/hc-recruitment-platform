...
export const POST = async (req, res) => {
    try {
            ...
            if (answer) {
                ...
                if (q.type === 'CHECKBOX' && q.options.length > 0) {
                    ...
                    if (Array.isArray(answer))
                        ...
                        selected = answer.split(',').map(s => s.trim());
                    else
        ...
        for (const recruiter of recruiters) {
            await createNotification(recruiter.id, "New Application Submitted", `${candidate.name} applied for the ${candidate.department} department.`);
        }
